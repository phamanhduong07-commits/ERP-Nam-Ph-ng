...
import {
  BankOutlined, CarOutlined, CheckCircleOutlined, DownOutlined, FileExcelOutlined,
...
export default function NganHangPage() {
  ...
  const { data: receiptData, isLoading: receiptLoading } = useQuery({
    queryKey: ['receipts-ck', tuNgay, denNgay, filterTrangThai, filterPhapNhan, filterPhanXuong, pageReceipt, searchQ],
  ...
  const { data: paymentData, isLoading: paymentLoading } = useQuery({
    queryKey: ['payments-ck', tuNgay, denNgay, filterTrangThai, filterPhapNhan, filterPhanXuong, pagePayment, searchQ],
      ...
      onOk: async () => {
        try {
          if (activeTab === 'thu') {
            ...
            queryClient.invalidateQueries({ queryKey: ['receipts-ck'] })
          } else {
            await Promise.all(ids.map(id => paymentApi.approve(id)))
            setSelectedPaymentKeys([])
            queryClient.invalidateQueries({ queryKey: ['payments-ck'] })
          }
