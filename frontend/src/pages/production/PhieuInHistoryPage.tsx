...
function TabLichSuIn() {
  ...
  const columns = [
    ...
    { title: 'Khổ (cm)', dataIndex: 'rong', width: 85, align: 'right' as const, render: (v: number | undefined) => v != null ? v : '—' },
    { title: 'Dài (cm)', dataIndex: 'dai', width: 85, align: 'right' as const, render: (v: number | undefined) => v != null ? v : '—' },
...
function TabScanByLoai({ loai, label }: { loai: 'can_mang' | 'xa', label: string }) {
  ...
  const handleExport = () => {
    exportToExcel(`lich-su-${loai}-${dayjs().format('YYYYMMDD')}`, [{
      name: label,
      headers: ['Ngày giờ', 'LSX', 'Tên hàng', 'Máy scan', 'Người SX', 'Khổ (mm)', 'Dài (mm)', 'SL TP', 'm²', 'Đơn giá', 'Tiền lương'],
      rows: filtered.map(r => [
  ...
  const columns = [
    ...
    { title: 'Khổ (mm)', dataIndex: 'rong', width: 85, align: 'right' as const, render: (v: number | undefined) => v ?? '—' },
    { title: 'Dài (mm)', dataIndex: 'dai', width: 85, align: 'right' as const, render: (v: number | undefined) => v ?? '—' },
