...
function deltaColor(value: number): string {
  if (value > 0) return '#cf1322'   // red — over budget / over usage
  if (value < 0) return '#3f8600'   // green — under budget / under usage
...
function DeltaCell({ value }: { value: number }) {
  ...
  return <span style={{ color, fontWeight: value !== 0 ? 600 : undefined }}>{prefix}{value.toLocaleString('vi-VN')}</span>
}
...
export default function CostAnalysisPage() {
  ...
  return (
    <div style={{ padding: '16px 24px' }}>
      <Title level={4} style={{ marginBottom: 16 }}>Phân tích chi phí / Chênh lệch</Title>

      {/* Search bar */}
      <Card style={{ marginBottom: 16 }}>
        <Space wrap>
          <Input
            placeholder="Nhập KHSX ID..."
            value={inputId}
      ...
      {error && !isLoading && (
        <Card>
          <Empty
            description={
              <Text type="danger">
                Không tải được dữ liệu. Kiểm tra lại ID hoặc kết nối server.
              </Text>
            }
          />
        </Card>
      ...
      {!khsxId && !isLoading && (
        <Card>
          <Empty description="Nhập ID kế hoạch sản xuất để xem phân tích chi phí" />
        </Card>
      )}
